...
public class MemberService {
    ...
    public List<Member> findDynamicRandomMembersByChannelId(Long memberId, Long channelId) {
        Long memberCount = countMemberByChannelId(channelId);
        if (memberCount < 8) throw BusinessException.of(Error.NOT_ENOUGH_MEMBER);
        int limit = ShuffleEstimationConstants.determineLimit(memberCount);
        return memberRepository.findRandomMembersByChannelId(memberId, channelId, limit);
    }
